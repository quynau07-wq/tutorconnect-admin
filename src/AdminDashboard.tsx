...
import {createUser, createStaff, deleteStaff, listStaff, listUsers, ManagedUser, StaffAdmin, Tutor, updateStaff, updateTutorStatus, updateUserProfile, updateUserStatus} from './api';
...
const AdminDashboard = ({user, role}: Props) => {
  ...
  <main className="content"><header><div><p className="eyebrow">{role === 'super_admin' ? 'Toàn quyền vận hành' : 'Quản lý người dùng'}</p><h1>{tab === 'operations' ? '\u0054heo d\u00f5i h\u1ecdc t\u1eadp' : tab === 'contracts' ? '\u0047\u00f3i h\u1ecdc v\u00e0 h\u1ee3p \u0111\u1ed3ng' : tab === 'review' ? 'Duyệt hồ sơ gia sư' : tab === 'parents' ? 'Quản lý người dùng' : tab === 'tutors' ? 'Quản lý gia sư' : tab === 'staff' ? 'Admin phụ' : tab === 'jobs' ? 'Bài đăng tìm gia sư' : 'Doanh thu'}</h1><p className="muted">{tab === 'revenue' ? 'Theo dõi doanh số trung tâm và tỷ trọng theo gia sư.' : 'Xem chi tiết, theo dõi và quản lý tài khoản.'}</p></div><div className="status-chip"><span /> Hệ thống đang hoạt động</div></header>{error && <div className="error-banner">{error}</div>}
    ...
    {tab === 'revenue' && role === 'super_admin' && <RevenueDashboard />}
